...
def main(train_args):
    ...
    criterion = torch.nn.CrossEntropyLoss(size_average=False, ignore_index=voc.ignore_label).to(device)
...
def train(train_loader, net, criterion, optimizer, epoch, train_args):
    ...
    for i, data in enumerate(train_loader):
        ...
        loss = criterion(outputs, labels) / N # try `size_average = True` later
        loss.backward()
        optimizer.step()
...
def validate(val_loader, net, criterion, optimizer, epoch, train_args, restore_transform, visualize):
    net.eval()
